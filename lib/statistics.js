...
// 𝛴f(xᵢ)vᵢ
{
    ...
    return this.bins.rsum(b=>f(b-this.step/2)*(this.data[b]||0))
...
Histogram.prototype.convolve = function(h)
{
...
{
    return new Histogram(s).fromList(this.toList())
}

Histogram.prototype.map = function(f)
{
    return new Histogram(this.step).fromList(this.toList().map(f))
}
...
var a = new Histogram().fromMap({1:1,2:1,3:1,4:1})
var b = new Histogram().fromMap({1:1,2:1,3:1,4:1})
 ...
 { get : function ()
   {
       ...
       {
	   if (this.PDF(b) > 0)
	       res += this.PDF(b)*ln(this.PDF(b))
       }
       return -res*this.step
   }
 }
)
...
function Binomial(n,p)
{
	...
	if (opts.p) return Binomial(round(h.mean/opts.p),opts.p) 
...
function GG1(gi,go,e)
{
	...
	var o = out()
	if (q.length==0)
	    q.push(pClient(t,t + o,n++,o))
	else if (impatient())
	{
	    var fst = q.shift()
	    q = q.map(c => [tIn(c),tOut(c)+o,cN(c),tServ(c)])
	    q.unshift(pClient(t, tOut(fst)+o, n++))
	    q.unshift(fst)
	}
	else
	    q.push(pClient(t,tOut(q.last()) + o,n++,o))
	newt = newComer()
